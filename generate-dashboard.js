...
const fmt = (h) => h === null ? 'n/a' : h < 24 ? h.toFixed(1)+'h' : (h/24).toFixed(1)+'d';

async function fetchAllTickets() {
  console.log('Fetching tickets from Freshservice API...');
  ...
  while (!done && page <= 120) {
    try {
      const res = await axios.get(`${baseURL}/tickets`, {
        auth,
        params: { per_page: 100, page, updated_since: '2026-03-01T00:00:00Z', order_by: 'created_at', order_type: 'desc' }
      });
      const tickets = res.data.tickets || [];
      if (!tickets.length) break;

      for (const t of tickets) {
        if (new Date(t.created_at) < MAR_CUTOFF) { done = true; break; }
        if (t.group_id === HD_GROUP) all.push(t);
      }

      const links = res.headers.link || '';
      if (!links.includes('rel="next"')) break;
      page++;
    } catch (e) {
  ...
  console.log(`Fetched ${all.length} HD tickets across ${page} pages`);
  return all;
}
...
function calcStats(tickets) {
  ...
  return {
    ...
    avgFRT: avgFRT ? +avgFRT.toFixed(1) : null,
    avgTTR: avgTTR ? +avgTTR.toFixed(1) : null,
...
function getWeeks(junTickets) {
  ...
  while (weekStart <= now) {
    ...
    if (wt.length > 0) {
      ...
      weeks.push({
        label: `Wk ${weeks.length+1}\nJun ${weekStart.getDate()}–${endDay}`,
        shortLabel: `Jun ${weekStart.getDate()}–${endDay}`,
        ...calcStats(wt)
      });
    }
...
function getDays(junTickets) {
  ...
  while (d <= now) {
    ...
    days.push({ label: `Jun ${d.getDate()}`, isWeekend: [0,6].includes(d.getDay()), ...s });
...
function buildHTML(data) {
  ...
  const wk1 = weekly[0] || {};
  const wk2 = weekly[1] || {};
  const wkColors = weekly.map((_,i) => i===0?'#2B5CE6':i===1?'#1A7A52':i===2?'#9B5DE5':i===3?'#F15BB5':'#00BBF9');
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .wk-val{font-family:var(--mono);font-weight:500;color:#fff}
  .wk-up{color:#6EE7B7;font-size:11px;font-weight:500}
  .wk-dim{opacity:.45}
  ...
  thead tr{border-bottom:1px solid var(--border);background:#FAFAF8}
  th{text-align:left;padding:10px 14px;font-family:var(--mono);font-size:10px;letter-spacing:.08em;text-transform:uppercase;color:var(--text-3);font-weight:500}
  ...
  tr.warn td:first-child{border-left:3px solid var(--amber)}
  ...
  .legend{display:flex;flex-wrap:wrap;gap:16px;margin-bottom:12px;font-size:12px;color:var(--text-2)}
  .li{display:flex;align-items:center;gap:6px}.sw{width:10px;height:10px;border-radius:2px}.sl{width:14px;height:2px}
...
<div class="header">
  <div class="header-inner">
    <div>
      <div class="eyebrow">Patriot Growth Insurance Services · IT Operations</div>
      <h1>Help Desk <span>Performance</span> Dashboard</h1>
    </div>
    <div>
      <div class="auto-badge">⚡ Auto-updated · ${updated}</div>
      <div class="header-sub">Freshservice · HD Team · All statuses incl. pending · Runs nightly</div>
    </div>
  </div>
</div>
...
<div class="page">
  ...
  <div class="section">
    ...
    <div class="month-banner">
      <div class="month-banner-inner">
        <div>
          ...
          <div class="mb-heading">Strong improvement — team is gaining momentum week over week</div>
          <div class="mb-kpis">
            <div><div class="mb-kpi-label">Tickets</div><div class="mb-kpi-value">${jun.total}</div><div class="mb-kpi-sub">month to date</div></div>
            <div><div class="mb-kpi-label">FCR rate</div><div class="mb-kpi-value">${jun.fcr}%</div><div class="mb-kpi-sub">target 70% ✓</div></div>
            <div><div class="mb-kpi-label">SLA</div><div class="mb-kpi-value">${jun.overSLA}%</div><div class="mb-kpi-sub">target 90%</div></div>
        ...
        <div class="wk-grid">
          ...
          ${weekly.length < 4 ? Array(4-weekly.length).fill(0).map((_,i) => `
          <div class="wk-card wk-dim">
            <div class="wk-label">Week ${weekly.length+i+1} · Coming</div>
            <div style="font-size:12px;color:rgba(255,255,255,.4);margin-top:8px">Not started yet</div>
          </div>`).join('') : ''}
        </div>
      </div>
    </div>
    ...
    <div class="chart-grid-4">
      ...
      <div class="chart-card"><div class="chart-label">Avg response time (h)</div><div style="position:relative;height:160px"><canvas id="wk_frt"></canvas></div></div>
      <div class="chart-card"><div class="chart-label">Avg resolution time (h)</div><div style="position:relative;height:160px"><canvas id="wk_ttr"></canvas></div></div>
  ...
  <div class="section">
    ...
    <div class="kpi-grid-5">
      <div class="kpi-card"><div class="kpi-label">Total tickets</div><div class="kpi-value">${overall.total.toLocaleString()}</div><div class="kpi-sub">incl. pending & WIP</div></div>
      <div class="kpi-card green"><div class="kpi-label">FCR rate</div><div class="kpi-value">${overall.fcr}%</div><div class="kpi-sub">target 70%</div><div class="kpi-delta dg">✓ Above target</div></div>
      <div class="kpi-card ${overall.overSLA >= 90 ? 'green' : 'amber'}"><div class="kpi-label">SLA compliance</div><div class="kpi-value">${overall.overSLA}%</div><div class="kpi-sub">target 90%</div><div class="kpi-delta ${overall.overSLA >= 90 ? 'dg' : 'da'}">${overall.overSLA >= 90 ? '✓ On target' : '⚠ Below target'}</div></div>
  ...
  <div class="section">
    ...
    <div class="table-card">
      <table>
        <thead><tr>
          <th>Month</th><th class="r">Total</th><th class="r">Resolved</th><th class="r">Pending</th>
          <th class="r">Still open</th><th class="r">FCR</th><th class="r">SLA</th>
          <th class="r">Avg response</th><th class="r">Avg resolution</th><th>Status</th>
        </tr></thead>
        <tbody>
  ...
  <!-- DAILY CHARTS -->
  <div class="section">
    <div class="section-header"><span class="section-label">Daily trends — June 2026 · all statuses</span><div class="section-rule"></div></div>
    <div class="legend">
    ...
    <div class="chart-card-full">
      <div class="chart-label">SLA first response rate (%)</div>
      <div style="position:relative;height:200px"><canvas id="slaChart"></canvas></div>
    </div>
    ...
    <div class="insight"><strong>Dashboard auto-updates nightly via GitHub Actions.</strong> No manual steps needed — data is pulled directly from Freshservice every day at 2:00 AM ET and the page rebuilds automatically.</div>
  </div>
...
const mkOpts=(yMin,yMax,suffix)=>({responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:i=>i.raw!==null?' '+i.raw+suffix:' no data'}}},scales:{x:{ticks:{color:'#9B9890',font:{size:10},autoSkip:false,maxRotation:45},grid:{display:false}},y:{min:yMin,max:yMax,ticks:{color:'#9B9890',font:{size:10},callback:v=>v+suffix},grid:{color:'rgba(0,0,0,0.05)'}}}});
...
async function main() {
  ...
  const getMonth = (mo, yr) => all.filter(t => {
  ...
  console.log(`Dashboard generated successfully — ${html.length} chars`);
}
